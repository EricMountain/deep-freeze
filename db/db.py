...
class Database():
   def __init__(self):
      ...
      self.connection.execute('''create table if not exists file_archive_records (
                              ...
                              ''')
   #   self.connection.execute('''create unique index if not exists host_file_backup_records_1 on host_file_backup_records (
   #                               file_host_fqdn,
   #                               file_backup_root,
   #                               file_path,
   #                              )
      ...
      with self.connection:
         ...
         query = '''
                  ...
                  update set
                     ...
                     sweep_mark = 'present'
                  '''
         cursor.execute(query, (client_fqdn, backup_root, rel_path, metadata.st_size, datetime_str, status, metadata.st_size, datetime_str,
